...
class Forcing:
    ...
    #def __init__(self, forcingtype, time, pad, extendstart):
        if forcingtype == 'aggTS':
            ...
            self.MLD = IndForcing('mld', 'X', k=5, s=100, kind="spline", forctype=forcingtype, WOA=True, Lat=lat, Lon=lon, RBB=rbb)
            self.NOX = IndForcing('n0x', 'X', k=5, s=None, kind="PWPoly", forctype=forcingtype, WOA=True, Lat=lat, Lon=lon, RBB=rbb)
            #self.SiOX = IndForcing('SiOH', 'Forcing/SiOH_filler/SiOH_R1.csv', k=5, s=None, kind="PWPoly", forctype=forcingtype)
            self.SST = IndForcing('sst', 'X', k=5, s=5, kind="spline", forctype=forcingtype, WOA=True, Lat=lat, Lon=lon, RBB=rbb)
            self.PAR = IndForcing('par', 'X', k=5, s=5, kind="spline", forctype=forcingtype, WOA=True, Lat=lat, Lon=lon, RBB=rbb)
