...
from runs.modelrun_fullTS_PFTs_4 import timedays, outarray, ms
...
outarray_reg1,timedays_reg1 = ms.physics.forcing.NOX.returnModelOut_Regime(outarray,timedays, regime=1)
outarray_reg2,timedays_reg2 = ms.physics.forcing.NOX.returnModelOut_Regime(outarray,timedays, regime=2)

timedays_reg1 = np.mod(timedays_reg1, 365)
timedays_reg2 = np.mod(timedays_reg2, 365)
...
def plotstuff(ms,outarray, zuplot, muplot, regime, plot):
    ...
    plt.setp(([ax1,ex1], [ax1s,ex1s], [ax2,ex2], [ax3,ex3], [ax4,ex4]), xticks=[1, 60, 120, 180, 240, 300, 365])
    ...
    if plot == 'model':
        plotregimex(ax1,ax1s,ax2,ax3,ax4,ms1, timedays_reg1, outarray_reg1,regime)

        plotregimex(ex1,ex1s,ex2,ex3,ex4,ms2,timedays_reg2, outarray_reg2,'Regime 2')
